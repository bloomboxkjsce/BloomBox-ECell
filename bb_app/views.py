...
from .models import Events, EventRegistrations, EventRegistrationsHackathon, ESummitRegistration, ESummitRegistrationHackathon, Campus, Message, ESummitMessage, Plandemic
...
def campus(request):
    ...
    context = {'companies': companies, 'active_companies': active_companies,
               'converted_to_startup': converted_to_startup, 'noSaturation': noSaturation}
...
def registration(request, event):
    if request.method == "POST":
        ...
        if eve.eventType == "Hackathon":
            ...
            var = EventRegistrationsHackathon.objects.create(title=eve, leaderName=leaderName, leaderEmail=leaderEmail,
                                                             leaderContact=leaderContact, college=college, nameOfTeam=nameOfTeam, nameOfMembers=nameOfMembers)
            var.save()
            name = leaderName
        else:
            ...
            var = EventRegistrations.objects.create(
                title=eve, name=name, email=email, contact=contact, college=college, branch=branch, year=year)
            var.save()
        ...
        events = Events.objects.all().order_by("-date")
        context = {'events': events, 'registered': True,
                   'registeredEvent': event, 'name': name}
        return render(request, 'events.html', context)
    else:
...
def plandemic(request):
    if request.method == "POST":
        ...
        nameOfMembers = request.POST["nameOfMembers"]
        print(leaderName)
        var = Plandemic.objects.create(leaderName=leaderName, leaderEmail=leaderEmail, leaderContact=leaderContact,
                                       college=college, nameOfTeam=nameOfTeam, nameOfMembers=nameOfMembers)
        var.save()
        name = leaderName
        context = {'registered': True, 'name': name}
        return render(request, 'Plandemic.html', context)
    else:

        return render(request, 'Plandemic.html')
...
def eSummitRegistration(request, eventId):
    if (eventId >= 1 and eventId <= 8):
        eventMappings = {1: 'Ennovate', 2: 'What Went Wrong?', 3: 'Think Tank', 4: 'Brand Wars', 5: 'Sociathon',
                         6: 'Lets talk Finance', 7: 'The Subtle Art of Influencership', 8: 'Mock Crypto Trading Contest'}
        if request.method == "POST":
            ...
            if(eventId in [6, 7, 8]):
                ...
                branch = request.POST["branch"]
                year = request.POST["year"]
                esummit_var = ESummitRegistration.objects.create(
                    email=email, eventId=eventId, eventName=eventName, name=name, contact=contact, college=college, branch=branch, year=year)
                esummit_var.save()
                reg_conf = "Thank you " + str(name) + ". You have been registered for " + str(
                    eventName) + ". You will be contacted by our team shortly."
            else:
                nameOfTeam = request.POST["nameOfTeam"]
                leaderEmail = request.POST["leaderEmail"]
                leaderName = request.POST["leaderName"]
                ...
                nameOfMembers = request.POST["nameOfMembers"]
                esummit_var = ESummitRegistrationHackathon.objects.create(eventId=eventId, eventName=eventName, nameOfTeam=nameOfTeam, leaderEmail=leaderEmail, leaderName=leaderName,
                                                                          leaderContact=leaderContact, college=college, alternateEmail=alternateEmail, alternateContact=alternateContact, nameOfMembers=nameOfMembers)
                esummit_var.save()
                reg_conf = "Thank you " + str(leaderName) + ". You have been registered for " + str(
                    eventName) + ". You will be contacted by our team shortly."
            if(eventId in [6, 7, 8]):
                eventType = 1
            else:
            ...
            context = {'eventId': eventId, 'eventType': eventType,
                       'eventName': eventName, 'reg_conf': reg_conf}
            ...
            reg_conf = ""
            eventName = eventMappings.get(eventId)
            closed_events = [1, 2, 3, 4, 5, 6, 7, 8]
            context = {'eventId': eventId, 'eventType': eventType, 'eventName': eventName,
                       'reg_conf': reg_conf, 'closed_events': closed_events}
            return render(request, 'eSummitRegistration.html', context)
    else:
        return render(request, 'eSummit.html')
...
def eSummitQuery(request):
    if request.method == "POST":
        ...
        contact = request.POST["number"]
        message = request.POST["textarea"]
        query_var = ESummitMessage.objects.create(
            name=name, email=email, contact=contact, message=message)
    return render(request, 'eSummit.html')
